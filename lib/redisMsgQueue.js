...
RedisMsgQueue.prototype.enqueue = function (group, task, callback) {
    ...
    return multi.exec(function (error, replies) {
        ...
        if (!_.isFinite(replies[1]) || replies[1] <= 0) {
            console.warn('RedisMsgQueue.enqueue()  The redis msg queue ' + msgQueueName +
                         ' was unable to add the task for the message group ' + msgGroupName +
                         ' - the hast reported a zero or negative number of tasks adding this task to the number, reply ' + util.inspect(replies[1]) + '.');
            return callback('The redis msg queue ' + queueName +
                            ' was unable to add the task for the message group ' + group + '.');
        }
...
RedisMsgQueue.prototype._process = function (workerName, msgWorker, msgQueue) {
    console.log('RedisMsgQueue._process()  The redis msg worker.');
    ...
    return msgQueue.redisClient.srandmember('set_' + msgQueue.msgQueueName, function (error, reply) {
        ...
        // Setup the watch keys for the app:queue:group hash - this is the 'reply' from the last query
        var randAppQueueGroup = reply;
        return msgQueue.redisClient.watch('hash_' + randAppQueueGroup, function(error, reply) {
            if (error || reply !== 'OK') {
                console.warn('RedisMsgQueue._process()  The redis msg queue ' + msgQueue.msgQueueName +
            ...
            return msgQueue.redisClient.hmget('hash_' + randAppQueueGroup, 'tasks', 'locked', function (error, replies) {
                ...
                return multi.exec(function (error, replies) {
                    ...
                    task = _.isObject(replies[0]) ? replies[0] : JSON.parse(replies[0]);
                    return msgWorker(undefined, task, function(error, task) {
                        if (randAppQueueGroup !== _setMsgGroupName(msgQueue.appName, msgQueue.queueName)) {
