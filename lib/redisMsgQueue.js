...
// TODO - Fix This!  I need to add something to gracefully shutdown after all workers really close
    ...
    return this._timeoutProcess(workerName, worker, this, this.pollInterval);
};

// ----------------------------------------------------------------------------------------------------
// The msg queue timed task processing (private)
// ----------------------------------------------------------------------------------------------------
RedisMsgQueue.prototype._timeoutProcess = function (workerName, msgWorker, msgQueue, timeout) {
    return setTimeout(function () {
        return msgQueue._process(workerName, msgWorker, msgQueue);
    }, timeout);
};
...
// ----------------------------------------------------------------------------------------------------
RedisMsgQueue.prototype._process = function (workerName, msgWorker, msgQueue) {
    ...
    return msgQueue.redisClient.srandmember('set_' + msgQueue.msgQueueName, function (error, reply) {
        if (error) {
            ...
            return msgQueue._timeoutProcess(workerName, msgWorker, msgQueue, msgQueue.pollInterval);
        }
        if (!_.isString(reply) || _.isEmpty(reply)) {
            return msgQueue.redisClient.watch('set_' + msgQueue.msgQueueName, function(error, reply) {
                if (error || reply !== 'OK') {
                    console.warn('RedisMsgQueue._process()  The redis msg queue ' + msgQueue.msgQueueName +
                                 ' was unable to watch the app:queue set ' + msgQueue.msgQueueName + ' - reply ' +
                                 util.inspect(reply) + ', error ' + util.inspect(error) + '.');
                    if (!error) {
                        return msgQueue.redisClient.unwatch(function (error, reply) {
                            if (error || reply !== 'OK') {
                                console.warn('RedisMsgQueue._process()  The redis msg queue ' + msgQueue.msgQueueName +
                ...
                return msgQueue.redisClient.scard('set_' + msgQueue.msgQueueName, function (error, reply) {
                    ...
                    if (reply === 0) {
                        ...
                        var multi = msgQueue.redisClient.multi();
                        ...
                        return multi.exec(function (error, replies) {
                            ...
                            if (_.isNull(replies)) {
                                return msgQueue._timeoutProcess(workerName, msgWorker, msgQueue, 0);
                            }
        ...
        return msgQueue.redisClient.watch('key_' + randAppQueueGroup, function(error, reply) {
            if (error || reply !== 'OK') {
                console.warn('RedisMsgQueue._process()  The redis msg queue ' + msgQueue.msgQueueName +
            ...
            return msgQueue.redisClient.hget('hash_' + randAppQueueGroup, 'tasks', function (error, reply) {
                if (error) {
                    console.warn('RedisMsgQueue._process()  The redis msg queue ' + msgQueue.msgQueueName + ' was unable to read the' +
                                 ' number of tasks in the app:queue:group hash ' + randAppQueueGroup + ' - reply ' + util.inspect(reply) +
                                 ', error ' + util.inspect(error) + '.');
                }
                // Check for non-execution due to watched key changing
                var tasks = null;
                if (_.isNull(reply)) {
                } else if (_.isFinite(reply)) {
                    tasks = reply;
                } else if (_.isString(reply) && !_.isEmpty(reply) && _.isFinite(parseInt(reply))) {
                    tasks = parseInt(reply);
                } else {
                    console.warn('RedisMsgQueue._process()  The redis msg queue ' + msgQueue.msgQueueName + ' was unable to read the' +
                                 ' number of tasks in the app:queue:group hash ' + randAppQueueGroup + ' - reply ' + util.inspect(reply) +
                                 ', error ' + util.inspect(error) + '.');
                }
            //return msgQueue.redisClient.hget('hash_' + randAppQueueGroup, 'locked', function (error, replies) {
            /*
            return msgQueue.redisClient.hmget('hash_' + randAppQueueGroup, 'tasks', 'locked', function (error, replies) {
                if (error || !_.isArray(replies) || replies.length !== 2 ||
                    (!_.isNull(replies[0]) && !_.isFinite(replies[0]) && !_.isString(replies[0])) ||
                    (!_.isNull(replies[1]) && !_.isDate(replies[1]) && !_.isString(replies[1]))) {
                    console.warn('RedisMsgQueue._process()  The redis msg queue ' + msgQueue.msgQueueName + ' was unable to read the' +
                ...
                var tasks = _.isNull(replies[0]) ? 0 : _.isFinite(replies[0]) ? replies[0] :
                            _.isString(replies[0]) ? parseInt(replies[0]) : undefined
                  , locked = _.isNull(replies[1]) ? null : _.isDate(replies[1]) ? replies[1] :
                             _.isString(replies[1]) ? Date.parse(replies[1]) : undefined;
                if (_.isDate(locked)) {
                    return msgQueue.redisClient.unwatch(function (error, reply) {
                        if (error || reply !== 'OK') {
                            console.warn('RedisMsgQueue._process()  The redis msg queue ' + msgQueue.msgQueueName +
                        ...
                        return msgQueue._timeoutProcess(workerName, msgWorker, msgQueue, 1);
                    });
                }
                */
                if (tasks === 0) {
...
console.log('test point 4');
                    ...
                    return msgWorker(task, function(error, myTask) {
                        if (randAppQueueGroup !== _setMsgGroupName(msgQueue.appName, msgQueue.queueName)) {
                            return msgQueue.redisClient.hdel('hash_' + randAppQueueGroup, 'locked', function (error, reply) {
                                if (error) {
                                    console.warn('RedisMsgQueue._process()  The redis msg queue ' + msgQueue.msgQueueName +
                                                 ' was unable to read a task from the msg group ' + randAppQueueGroup +
